...
records the (actor, growth stage, facing) cells whose art came out mirrored vs its
filename, and the importer loads the mirror-partner FILE for those cells. We fold that
remap in identically: HeadCx / HeadW come from the file the importer actually loads,
while FrontX is placed on the side the dino VISUALLY faces (i.e. the logical Dir8).

The remap is PER GROWTH STAGE (DinoDigger-3yb). It used to be per actor and adult-only,
which is why this script used to carry a note about `StagePath` deliberately skipping it;
that asymmetry was the bug, not a design, and it is gone. Every path this script resolves
now goes through the same (folder, stage) lookup the importer uses, so re-baking after a
remap change moves the anchors with the art automatically.
...
# Mirror of GeneratedArtImporter.FlippedFacingPairs, keyed by (folder, stage) and by the
# right-side member (E/SE/NE) of each flipped pair. PER STAGE since DinoDigger-3yb: each
# growth stage is its own generation, so handedness does not survive the growth ladder
# (stegosaurus is snout-LEFT at E as an adult and a kid, snout-RIGHT as a baby). KEEP IN
# STEP with the importer's table — the two must agree or the anchors are measured off a
# file the game does not display.
#
# This is the third copy of one measured fact, so it is GUARDED rather than trusted:
# `python3 Tools/verify_facing_art.py --check` re-measures the source PNGs and fails if
# this dict, the importer's table, or the baked Assets/Scripts/Config/FacingManifest.cs
# has drifted from the art. Change any of them and run that.
FLIPPED_FACINGS = {
    ("backhoe", "adult"): {"SE", "NE"},
    ("trex", "adult"): {"SE"},
    ("triceratops", "adult"): {"SE"},
    ("stegosaurus", "adult"): {"E", "SE"},
    ("ankylosaurus", "adult"): {"E", "SE", "NE"},
    ("velociraptor", "adult"): {"SE"},

    ("triceratops", "kid"): {"SE"},
    ("stegosaurus", "kid"): {"E", "SE", "NE"},
    ("ankylosaurus", "kid"): {"E", "SE", "NE"},

    ("trex", "baby"): {"SE"},
    ("stegosaurus", "baby"): {"SE", "NE"},
    ("ankylosaurus", "baby"): {"SE"},
}
...
def file_dir(folder: str, stage, d: str) -> str:
    ...
    if pair_key(d) in FLIPPED_FACINGS.get((folder, stage or "adult"), ()):
        return mirror_dir(d)
    return d
